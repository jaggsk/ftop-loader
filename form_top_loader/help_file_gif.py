...
def play_help_gif():
    ...
    root_help.iconphoto(False, PhotoImage(file='.\\Config\\doge_icon.PNG',master = root_help))
    #root = Toplevel()
    ...
    frames = [PhotoImage(file='.\\Config\\ok_boomer.gif',format = 'gif -index %i' %(i), master = root_help) for i in range(10)]
    def update(ind):
        frame = frames[ind]
        ind += 1
        #print(ind)
